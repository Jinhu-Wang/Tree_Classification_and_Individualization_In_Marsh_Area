...
FoxTree::FoxTree()
    : m_Points(nullptr), m_nNumPts(0), m_nTreeIndex(0), m_nVerticalResolution(0.0), m_nRadius(0.0), m_nMinPtSeeds(5)
//, m_Voxel(nullptr)
{
}
...
FoxTree::FoxTree(std::vector<Point3D> points, double radius, double verticalResolution, int minPtNum)
{
    ...
    for (size_t i = 0; i < points.size(); ++i)
    {
        ...
        if (points.at(i).x > this->m_nBBX.xMax)
            this->m_nBBX.xMax = points.at(i).x;
        if (points.at(i).y > this->m_nBBX.yMax)
            this->m_nBBX.yMax = points.at(i).y;
        if (points.at(i).z > this->m_nBBX.zMax)
            this->m_nBBX.zMax = points.at(i).z;

        if (points.at(i).x < this->m_nBBX.xMin)
            this->m_nBBX.xMin = points.at(i).x;
        if (points.at(i).y < this->m_nBBX.yMin)
            this->m_nBBX.yMin = points.at(i).y;
        if (points.at(i).z < this->m_nBBX.zMin)
            this->m_nBBX.zMin = points.at(i).z;
    }
}
...
void FoxTree::listFilesInDirectory(const std::string &directoryPath, std::vector<std::string> &outFiles)
{
    try
    {
...
std::vector<std::vector<int>> FoxTree::clusterPoints(double radius, std::vector<int> ptIndices)
{
    ...
    for (size_t i = 0; i < ptIndices.size(); ++i)
    {
        ...
        if (this->m_Points[ptID].isVisited)
            continue;
...
void FoxTree::outputTrees(std::string filename, std::map<int, TreeCluster> trees)
{
    ...
    /*for (int i = 0; i < trees.size(); ++i)
    {
        ...
        for (int j = 0; j < trees[i].ptIDs.size(); ++j)
        {
            int id = trees[i].ptIDs.at(j);
    ...
    for (const auto &[key, cluster] : trees)
    {
        int r, g, b;
        r = rand() % 255;
        g = rand() % 255;
        ...
        for (int j = 0; j < cluster.ptIDs.size(); ++j)
        {
            int id = cluster.ptIDs.at(j);
            Point3D pt = this->m_Points[id];
            fprintf(file, "%d %lf %lf %lf %d %d %d\n",
                    pt.treeID, pt.x, pt.y, pt.z,
                    r, g, b);
